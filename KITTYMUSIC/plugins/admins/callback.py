...
from KITTYMUSIC import YouTube, app
from KITTYMUSIC.core.call import KITTY as KITTY
from KITTYMUSIC.misc import SUDOERS, db
from KITTYMUSIC.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from KITTYMUSIC.utils.decorators.language import languageCB
from KITTYMUSIC.utils.formatters import seconds_to_min
from KITTYMUSIC.utils.inline import (
    close_markup,
    panel_markup_1,
    panel_markup_2,
    panel_markup_3,
...
from KITTYMUSIC.utils.inline.play import stream_markup
from KITTYMUSIC.utils.stream.autoclear import auto_clean
from KITTYMUSIC.utils.thumbnails import get_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await KITTY.pause_stream(chat_id)
        buttons = [
            [
                InlineKeyboardButton(
        ...
        await KITTY.resume_stream(chat_id)
        buttons_resume = [
            [
                InlineKeyboardButton(
        ...
        await KITTY.mute_stream(chat_id)
        ...
        await KITTY.unmute_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await KITTY.st_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await KITTY.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await KITTY.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await KITTY.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup2(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await KITTY.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup2(_, chat_id)
