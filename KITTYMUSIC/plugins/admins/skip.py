...
from KITTYMUSIC.utils.database import get_loop
from KITTYMUSIC.utils.decorators import AdminRightsCheck
from KITTYMUSIC.utils.inline.play import stream_markup, telegram_markup
from KITTYMUSIC.utils.stream.autoclear import auto_clean
from KITTYMUSIC.utils.thumbnails import gen_thumb
...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
                        ...
                        for x in range(state):
                            ...
                            if not check:
                                try:
                                    ...
                                    await KITTY.stop_stream(chat_id)
                                except:
                                    return
                                break
                    else:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await KITTY.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            await KITTY.skip_stream(chat_id, link, video=status)
        except Exception:
        ...
        try:
            await KITTY.skip_stream(chat_id, file_path, video=status)
        except Exception:
            return await mystic.edit_text(_["call_7"])
        ...
        try:
            await KITTY.skip_stream(chat_id, queued, video=status)
        except Exception:
            return await message.reply_text(_["call_7"])
        if videoid == "telegram":
            button = telegram_markup(_, chat_id)
